...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{dokumentacja_projektowa/images/Diagram ERD.png}
    \caption{Diagram ERD}
\end{figure}
...
Kod źródłowy został podzielony na niezależne pakiety funkcjonalne. Poniżej przedstawiono szczegółowy opis każdego z nich.

\paragraph{Pakiet \texttt{com.BINM.user}}
Ten moduł odpowiada za zarządzanie tożsamością użytkowników, autentykację oraz obsługę profili. Jest to fundament bezpieczeństwa aplikacji.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/uml_user_package.png}
    \caption{Diagram klas pakietu \texttt{com.BINM.user}}
\end{figure}
...
\begin{itemize}
    \item \texttt{listing}:
        \begin{itemize}
            ...
            \item \texttt{SearchService}: Serwis dedykowany do wyszukiwania (CQRS).
...
\begin{figure}[H]
    ...
    \caption{Diagram klas pakietu \texttt{com.BINM.listing}}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Diagram klas pakietu \texttt{com.BINM.messaging}}
\end{figure}

\paragraph{Pakiet \texttt{com.BINM.interactions}}
Moduł obsługujący interakcje użytkowników z ogłoszeniami, w szczególności system "Ulubionych".
...
\begin{itemize}
    ...
    \item \texttt{model}:
        \begin{itemize}
            ...
            \item \texttt{EntityType}: Enum określający typ polubionego obiektu (obecnie tylko LISTING).
        \end{itemize}
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Diagram klas pakietu \texttt{com.BINM.interactions}}
\end{figure}
...
\begin{itemize}
    ...
    \item \texttt{storage}:
        \begin{itemize}
            \item \texttt{ImageStorageClient}: Klient integrujący się z Azure Blob Storage SDK.
        \end{itemize}
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Diagram klas pakietu \texttt{com.BINM.media}}
\end{figure}
...
\begin{itemize}
    ...
    \item \texttt{JwtChannelInterceptor}: Specjalny interceptor dla kanałów WebSocket, który autoryzuje połączenie na podstawie tokenu JWT w momencie nawiązywania sesji (handshake).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Facade (Fasada):} Używana do uproszczenia interfejsów modułów i ukrycia szczegółów implementacyjnych.
    \begin{itemize}
    ...
    \item \textbf{Strategy (Strategia):} Wzorzec behawioralny, który pozwala na zdefiniowanie rodziny algorytmów i ich wymienne stosowanie.
    \begin{itemize}
    ...
    \item \textbf{Observer (Obserwator) / Publish-Subscribe:} Wzorzec, w którym obiekt (podmiot) powiadamia listę zależnych od niego obiektów (obserwatorów) o zmianie swojego stanu.
    \begin{itemize}
        \item \textbf{Zastosowanie:} Zrealizowany za pomocą mechanizmu Spring Events. Przykład: Gdy ogłoszenie zostaje zakończone w \texttt{ListingService}, publikowane jest zdarzenie \texttt{ListingFinishedEvent}. Serwis \texttt{InteractionService} nasłuchuje na to zdarzenie (\texttt{@EventListener}) i automatycznie usuwa to ogłoszenie z ulubionych. Dzięki temu moduły są luźno powiązane (loose coupling) - \texttt{ListingService} nie musi wiedzieć o istnieniu \texttt{InteractionService}.
    \end{itemize}

    \item \textbf{Builder (Budowniczy):} Wzorzec konstrukcyjny, który pozwala na tworzenie złożonych obiektów krok po kroku.
    \begin{itemize}
    ...
    \item \textbf{Repository (Repozytorium):} Wzorzec, który pośredniczy między warstwą domeny a warstwą mapowania danych.
    \begin{itemize}
        \item \textbf{Zastosowanie:} Interfejsy rozszerzające \texttt{JpaRepository} (np. \texttt{ListingRepository}) zapewniają abstrakcję nad dostępem do bazy danych, ukrywając zapytania SQL/JPQL.
    \end{itemize}
    ...
    \item \textbf{Dependency Injection (Wstrzykiwanie Zależności):} Wzorzec, w którym obiekty nie tworzą swoich zależności, ale otrzymują je z zewnątrz.
    \begin{itemize}
        \item \textbf{Zastosowanie:} Fundament frameworka Spring. Wszystkie serwisy, kontrolery i repozytoria są wstrzykiwane przez konstruktor (z użyciem \texttt{@RequiredArgsConstructor}), co ułatwia testowanie i zarządzanie cyklem życia komponentów.
    \end{itemize}
\end{itemize}
